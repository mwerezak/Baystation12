...
#define AIRLOCK_CRUSH_DIVISOR 8 // Damage caused by airlock crushing a mob is split into multiple smaller hits. Makes door crushing behave more like a "slow" crushing effect rather than e.g. high-speed weapon impacts.
...
/mob/living/airlock_crush(var/crush_damage)
	...
	var/protection = (100 - getarmor(null, "melee"))/100
	crush_damage *= protection

	for(var/i in 1 to AIRLOCK_CRUSH_DIVISOR)
		apply_damage(crush_damage / AIRLOCK_CRUSH_DIVISOR, BRUTE, null, 0)

	SetStunned(round(crush_damage / 8))
	SetWeakened(round(crush_damage / 8))

	var/turf/T = get_turf(src)
	
	if(prob(crush_damage * 2))
		T.add_blood(src)
...
/mob/living/silicon/robot/airlock_crush(var/crush_damage)
	return ..(round(crush_damage / CYBORG_AIRLOCKCRUSH_RESISTANCE)) //temporary until robot armor is implemented